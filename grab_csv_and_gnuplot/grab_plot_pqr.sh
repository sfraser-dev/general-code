...
function gnuplot_rd1 {
  ...
  echo "set terminal png size 1200, 800; set output '$PNGFILEOUT'; \
    ...
    set xlabel 'Frame Number'; set ylabel 'PQR'; set title 'PQA PQR Plot'; set grid; \
...
sed -n '/Frame<Ref/,$p' $CLEANNAME | sed '/Frame<Ref/d' | sed 1d | cut -f8 -d, > $DATFILE
...
AVGPQR=$(sed -n '/PQR/,$p' datfile_unix_no_empty_spaces_with_summary.dat | tail -1)
echo "AVGPQR=$AVGPQR"
