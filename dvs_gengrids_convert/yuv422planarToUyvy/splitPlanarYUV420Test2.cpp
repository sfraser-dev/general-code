...
int main(int argc, char**argv) {
	...
	char *inputname="/home/stewart/Videos/gengrids_720x480_planar422_615frms.yuv";
	char *outputnameBase="gengridsPl";
	...
	fin=NULL;
        ...
        if ((fout2d = fopen("zz2d.yuv", "wb")) == NULL){
          printf("Error in opening file %s!\n", outputname);
          exit(-1);
        }
        for (i=0; i<maxNoOfFrames; i++) {
	  if ((status = fwrite(out[i], 1, frameSize, fout2d)) != frameSize){
	    printf("Error in writing to file %s!\n", outputname);
	    exit(-1);
	  }	
        }
